...
const ANOMALY_DETAILS: Record<string, {
  ...
  A005: {
    rootCause: "Local event in the city centre is expected to drive a 35% footfall increase this evening. Existing schedule does not account for this micro-event.",
    recommendation: "Reallocate 2 colleagues from ambient restocking to checkout duties between 17:00 and 19:30 to maintain queue KPIs.",
    actionLabel: "Reallocate Till Staff",
    resolvedMessage: "Staff reallocation schedule updated. 2 colleagues moved to checkout for the peak period."
  }
};
...
export default function TodayPriorities() {
  ...
  useEffect(() => {
    ...
    // Add settings variables — in demo mode use super-sensitive thresholds to guarantee all anomalies appear
    const effectiveWow   = demoMode ? 2  : wowDeclineThreshold;
    const effectiveWaste = demoMode ? 2  : wasteSpikeThreshold;
    query.set('wowDeclineThreshold', effectiveWow.toString());
    query.set('wasteSpikeThreshold', effectiveWaste.toString());
  ...
  const getHeaderTitle = () => {
    if (role === 'store_manager') return `Today's Priorities — ${storeObj?.name || selectedStore}`;
    if (role === 'category_manager') return `Today's Priorities — ${activeCategory} Category`;
  ...
  const getHeaderSub = () => {
    ...
    return 'Enterprise national business overview';
  };
                  ...
                  >
                    <div className="anomaly-drawer-grid">
                      <div>
                        ...
                        <div style={{ fontSize: '0.8125rem', color: 'var(--text-primary)', lineHeight: 1.6, marginBottom: 12 }}>
