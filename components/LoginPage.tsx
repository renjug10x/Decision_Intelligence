...
import { ICON_PROPS_SM } from '@/lib/icons';

const ROLES: { id: Role; Icon: any; name: string; desc: string }[] = [
...
export default function LoginPage({ onLogin }: { onLogin: () => void }) {
  const { setRole, setApiKey, setIsAuthenticated } = useApp();
    ...
    <div className="login-bg">
      <div className="login-card">
        ...
        <div style={{ position: 'relative', marginBottom: 10 }}>
          <Lock
            size={15} strokeWidth={1.75} color="#4A5A7A"
          ...
          <input
            ...
            value={keyInput}
            onChange={e => { setKeyInput(e.target.value); setSkipKey(false); setError(''); }}
            disabled={skipKey}
        ...
        <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', marginBottom: 6 }}>
          <input
            type="checkbox"
            checked={skipKey}
            onChange={e => { setSkipKey(e.target.checked); setError(''); }}
        ...
        {error && (
          <div style={{
            color: 'var(--danger)', fontSize: '0.8125rem', marginBottom: 14,
            background: 'var(--danger-light)', padding: '9px 14px',
        ...
        <p style={{ textAlign: 'center', fontSize: '0.75rem', color: 'var(--text-muted)', marginTop: 20, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6 }}>
