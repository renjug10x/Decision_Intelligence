...
exports.CAMPAIGN_ACTIVATION_IDS = exports.CAMPAIGN_CHANNEL_IDS = exports.CAMPAIGN_SEGMENT_IDS = exports.CAMPAIGN_CATEGORY_IDS = exports.CAMPAIGN_CHANNELS = exports.CAMPAIGN_ACTIVATIONS = exports.CAMPAIGN_SEGMENTS = exports.CAMPAIGN_CATEGORIES = void 0;
exports.resolveCategory = resolveCategory;
exports.resolveSegment = resolveSegment;
exports.resolveChannel = resolveChannel;
exports.resolveActivation = resolveActivation;
exports.categoryLabel = categoryLabel;
exports.segmentLabel = segmentLabel;
exports.channelLabel = channelLabel;
exports.activationLabel = activationLabel;
exports.addressableReachShare = addressableReachShare;
exports.isActivationCompatible = isActivationCompatible;
...
/* ────────────────────────────────────────────────────────────────────────────
   Lookup helpers
   ──────────────────────────────────────────────────────────────────────────── */
const CATEGORY_BY_ID = new Map(exports.CAMPAIGN_CATEGORIES.map(c => [c.id, c]));
const SEGMENT_BY_ID = new Map(exports.CAMPAIGN_SEGMENTS.map(s => [s.id, s]));
const CHANNEL_BY_ID = new Map(exports.CAMPAIGN_CHANNELS.map(c => [c.id, c]));
const ACTIVATION_BY_ID = new Map(exports.CAMPAIGN_ACTIVATIONS.map(a => [a.id, a]));
...
function resolveCategory(value) {
    if (!value)
        return null;
    const direct = CATEGORY_BY_ID.get(value);
    if (direct)
        return direct;
    const token = normaliseToken(value);
    return (exports.CAMPAIGN_CATEGORIES.find(c => normaliseToken(c.id) === token ||
        normaliseToken(c.display_label) === token ||
        normaliseToken(c.catalogue_key) === token) || null);
}
function resolveSegment(value) {
    if (!value)
        return null;
    const direct = SEGMENT_BY_ID.get(value);
    if (direct)
        return direct;
    const token = normaliseToken(value);
    return (exports.CAMPAIGN_SEGMENTS.find(s => normaliseToken(s.id) === token || normaliseToken(s.display_label) === token) || null);
}
function resolveChannel(value) {
    if (!value)
        return null;
    const direct = CHANNEL_BY_ID.get(value);
    if (direct)
        return direct;
    const token = normaliseToken(value);
    return (exports.CAMPAIGN_CHANNELS.find(c => normaliseToken(c.id) === token || normaliseToken(c.display_label) === token) || null);
}
function resolveActivation(value) {
    if (!value)
        return null;
    const direct = ACTIVATION_BY_ID.get(value);
    if (direct)
        return direct;
    const token = normaliseToken(value);
    return (exports.CAMPAIGN_ACTIVATIONS.find(a => normaliseToken(a.id) === token || normaliseToken(a.display_label) === token) || null);
}
 ...
 */
function addressableReachShare(segment, channel) {
 ...
 */
function isActivationCompatible(channel, activation) {
    const ch = resolveChannel(channel);
    const act = resolveActivation(activation);
    if (!ch || !act)
        return true; // unclassified values make no compatibility claim
    return ch.compatible_activations.includes(act.id);
}
/** Working days before the slowest selected route can go live. */
function executionLeadTimeDays(channel, activations) {
 ...
 */
function evaluateSubsidyConfinement(segment, channel, activations) {
    ...
    if (statedButUnresolved) {
        return {
            ...
            confinable: false,
            reached_share: 1,
            reason: 'The audience is stated as free text, so the estate cannot resolve who is in it and cannot keep the offer from anyone else. The discount is paid across the whole base.'
        };
    }
    const ch = resolveChannel(channel);
    const addressableActivation = (activations || []).find(a => resolveActivation(a)?.addressable === true);
...
exports.CAMPAIGN_CHANNEL_IDS = exports.CAMPAIGN_CHANNELS.map(c => c.id);
exports.CAMPAIGN_ACTIVATION_IDS = exports.CAMPAIGN_ACTIVATIONS.map(a => a.id);
