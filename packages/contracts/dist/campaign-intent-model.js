...
exports.CDI01_FORBIDDEN_CALCULATION_KEYS = exports.CAMPAIGN_CANVAS_AREA_ORDER = void 0;
exports.isAssistantDrafted = isAssistantDrafted;
exports.assistantDraftedCount = assistantDraftedCount;
exports.buildCampaignIntentId = buildCampaignIntentId;
exports.createDefaultCampaignIntentDraft = createDefaultCampaignIntentDraft;
exports.validateCampaignIntentCore = validateCampaignIntentCore;
...
exports.projectCampaignIntentToCommercialIntent = projectCampaignIntentToCommercialIntent;
exports.assertNoFutureCdiCalculations = assertNoFutureCdiCalculations;
/** Whether a decision-context entry entered the record as an accepted assistant draft. */
function isAssistantDrafted(context, entry) {
    if (!context?.assistant_drafted_entries || !entry)
        return false;
...
function assistantDraftedCount(context) {
...
exports.CAMPAIGN_CANVAS_AREA_ORDER = [
    'CAMPAIGN_INTENT',
    'BASELINE_OBJECTIVE',
    'AUDIENCE_MARKET',
    'DECISION_CONTEXT'
 ...
 */
function campaignIntentKeyDigest(tenantId, sessionId) {
...
function buildCampaignIntentId(tenantId, sessionId) {
...
function createDefaultCampaignIntentDraft(tenantId, sessionId, options) {
...
function validateCampaignIntentCore(core) {
    const errors = [];
    if (!core.objective_type)
        errors.push('campaign_intent.objective_type is required');
    if (!core.intervention_posture)
        errors.push('campaign_intent.intervention_posture is required');
    if (!core.framing_question)
        errors.push('campaign_intent.framing_question is required');
    if (!core.category)
        errors.push('campaign_intent.category is required');
    if (!Array.isArray(core.sku_scope) || core.sku_scope.length === 0) {
        errors.push('campaign_intent.sku_scope must be a non-empty array');
    }
    if (core.intervention_posture === 'CONSIDER_PROMOTION' &&
...
function validateBaselineObjective(area) {
    const errors = [];
    if (!area.primary_metric)
        errors.push('baseline_objective.primary_metric is required');
    if (!area.target_direction)
        errors.push('baseline_objective.target_direction is required');
    return { valid: errors.length === 0, errors };
}
function validateAudienceMarket(area) {
    const errors = [];
    if (!area.region)
    ...
    if (area.timing_mode === 'KNOWN_DATES') {
        ...
        if (!area.planned_end)
            errors.push('audience_market.planned_end is required when timing_mode is KNOWN_DATES');
...
function validateDecisionContextArea(area) {
    const errors = [];
...
function validateCampaignIntent(intent, options) {
    ...
    if (typeof intent.synthetic_demo !== 'boolean')
        errors.push('Missing required field: synthetic_demo');
    if (!intent.source_system)
 ...
 */
function projectCampaignIntentToCommercialIntent(campaign, options) {
    ...
    return {
        ...
        discount_depth: typeof campaign.campaign_intent.provisional_discount_depth === 'number'
...
exports.CDI01_FORBIDDEN_CALCULATION_KEYS = [
...
function assertNoFutureCdiCalculations(payload) {
    const raw = JSON.stringify(payload || {});
    const offenders = exports.CDI01_FORBIDDEN_CALCULATION_KEYS.filter(k => raw.includes(`"${k}"`));
