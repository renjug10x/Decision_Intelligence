...
export interface ExternalSignalConnectorDescriptor {
  connector_id: string;                 // e.g. conn_planning_ref_01
  category: ExternalSignalCategory;
  display_name: string;
  provider_id: string;                  // opaque provider label — not a hard dependency
  supported_signal_types: CanonicalSignalType[];
  status: ExternalSignalConnectorStatus;
  synthetic_demo: boolean;
  adapter_version: string;              // e.g. esf3_adapter_v1.0.0
...
export interface ExternalSignalEnvelope {
  ...
  effective_at?: string;                // ISO 8601 UTC; defaults to observed_at
  baseline_value: number;
  observed_value: number;
  unit: string;
  confidence?: number;                  // 0–100; adapter default applied if omitted
  quality?: number;                     // 0–100; adapter default applied if omitted
  /** Opaque provider payload reference — never the raw vendor body. */
  provider_payload_ref?: string;
  source_system: string;                // e.g. reference_planning_adapter
...
export const EXTERNAL_SIGNAL_CATEGORIES: ExternalSignalCategory[] = [
  'PLANNING',
  'COMMERCE',
  'WEATHER',
  'EVENTS',
...
export function mapCategoryToSourceType(category: ExternalSignalCategory): SignalSourceType {
  switch (category) {
    case 'PLANNING':
...
export function validateExternalSignalEnvelope(
  envelope: Partial<ExternalSignalEnvelope>
): { valid: boolean; errors: string[] } {
  const errors: string[] = [];

  if (!envelope.envelope_id) errors.push('Missing required field: envelope_id');
  if (!envelope.connector_id) errors.push('Missing required field: connector_id');
  if (!envelope.category) errors.push('Missing required field: category');
  if (!envelope.tenant_id) errors.push('Missing required field: tenant_id');
  if (!envelope.signal_type) errors.push('Missing required field: signal_type');
  if (!envelope.entity_type) errors.push('Missing required field: entity_type');
  if (!envelope.entity_id) errors.push('Missing required field: entity_id');
  if (!envelope.observed_at) errors.push('Missing required field: observed_at');
  if (typeof envelope.baseline_value !== 'number') errors.push('Missing required field: baseline_value');
  if (typeof envelope.observed_value !== 'number') errors.push('Missing required field: observed_value');
  if (!envelope.unit) errors.push('Missing required field: unit');
  if (!envelope.source_system) errors.push('Missing required field: source_system');
  if (typeof envelope.synthetic_demo !== 'boolean') errors.push('Missing required field: synthetic_demo');
  if (!envelope.schema_version) errors.push('Missing required field: schema_version');

  if (envelope.category && !EXTERNAL_SIGNAL_CATEGORIES.includes(envelope.category)) {
...
export function validateExternalSignalIngestRequest(
  request: Partial<ExternalSignalIngestRequest>
): { valid: boolean; errors: string[] } {
  const errors: string[] = [];

  if (!request.tenant_id) errors.push('Missing required field: tenant_id');
...
export function validateExternalSignalConnectorDescriptor(
  descriptor: Partial<ExternalSignalConnectorDescriptor>
): { valid: boolean; errors: string[] } {
  const errors: string[] = [];

  if (!descriptor.connector_id) errors.push('Missing required field: connector_id');
  if (!descriptor.category) errors.push('Missing required field: category');
  if (!descriptor.display_name) errors.push('Missing required field: display_name');
  if (!descriptor.provider_id) errors.push('Missing required field: provider_id');
  if (!descriptor.status) errors.push('Missing required field: status');
  if (!descriptor.adapter_version) errors.push('Missing required field: adapter_version');
  ...
  if (typeof descriptor.synthetic_demo !== 'boolean') errors.push('Missing required field: synthetic_demo');

  if (descriptor.category && !EXTERNAL_SIGNAL_CATEGORIES.includes(descriptor.category)) {
 ...
 */
export function normaliseEnvelopeToEnterpriseSignal(
  envelope: ExternalSignalEnvelope,
  options?: { signal_id?: string; category_override?: SignalCategory }
): { ok: true; signal: EnterpriseSignal } | { ok: false; errors: string[] } {
  ...
  const signalValidation = validateEnterpriseSignal(signal);
