...
export interface ForecastStabilityAssessment {
  stability_score: number | null;               // 0 to 100 percentage, or null if indeterminate
  ...
  contributing_signal_refs: string[];           // Real Canonical EnterpriseSignal references
...
// ── P0-B: Demand Frontier & Trajectory Set ──────────────────────────────────
...
// ── P0-B: Decision Gap Intelligence ─────────────────────────────────────────
...
// ── Decision Window (Supporting P0-B) ───────────────────────────────────────
...
export interface DemandDecisionWindow {
  ...
  deadline_display: string | null;              // e.g. "Friday 14:00 UTC"
...
export interface DemandInterventionScenario {
  ...
  description: string;
  rationale: string;
  parameter_modifications: Partial<DecisionScenarioParameters> & { additional_capacity_units?: number };
...
// ── Assumption Provenance Inventory ─────────────────────────────────────────
 ...
 */
export type DemandInputProvenanceClass =
  | 'SYNTHETIC_OBSERVED'
