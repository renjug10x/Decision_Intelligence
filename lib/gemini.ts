...
function mockAskNLQ(question: string, role: string, dataContext: any): NLQResponse {
  ...
  return {
    answer: "Enterprise revenue is steady at £1.45M for the last 7 days (+0.8% WoW). Produce waste and dairy margin compression remain the key operational variances.",
...
export async function askNLQ(params: {
  ...
  const prompt = `You are the AI analytics engine for CogniX Enterprise Innovation Lab.
...
export async function generateBriefing(params: {
  ...
  const prompt = `You are generating a daily business briefing for the ${roleTitle}.
