...
import {
  ...
  MemorySearchRequest,
  validateEnterpriseMemoryCase
} from '../../../packages/contracts/src/index';
import { IMemoryRepository } from './repository';

export const CANONICAL_MEMORY_CASES: EnterpriseMemoryCase[] = [
  {
...
export class InMemoryMemoryRepository implements IMemoryRepository {
  private cases: Map<string, EnterpriseMemoryCase> = new Map();
  ...
  private seed() {
    CANONICAL_MEMORY_CASES.forEach(c => {
      this.cases.set(c.memory_id, { ...c });
    });
  }

  public registerMemoryCase(caseObj: EnterpriseMemoryCase): EnterpriseMemoryCase {
    const val = validateEnterpriseMemoryCase(caseObj);
    if (!val.valid) {
  ...
  public getMemoryCaseById(id: string): EnterpriseMemoryCase | null {
    const found = this.cases.get(id);
    return found ? { ...found } : null;
  }

  public queryMemoryCases(filter: { tenant_id?: string; category?: string; pattern_id?: string; limit?: number }): EnterpriseMemoryCase[] {
  ...
  public searchMemoryPrecedents(request: MemorySearchRequest): EnterpriseMemoryCase[] {
    let results = Array.from(this.cases.values());
    ...
    if (request.query) {
      ...
      results = results.filter(c =>
        c.situation_summary.toLowerCase().includes(q) ||
