...
exports.learningPatternRepository = exports.InMemoryLearningPatternRepository = exports.CANONICAL_LEARNING_PATTERNS = void 0;
const memory_store_1 = require("./memory-store");
exports.CANONICAL_LEARNING_PATTERNS = [
    {
...
class InMemoryLearningPatternRepository {
    patterns = new Map();
    constructor() {
        this.seed();
    }
    seed() {
        exports.CANONICAL_LEARNING_PATTERNS.forEach(p => {
            this.patterns.set(p.pattern_id, { ...p });
        });
    }
    getLearningPatternById(id) {
        const found = this.patterns.get(id);
        return found ? { ...found } : null;
    }
    queryLearningPatterns(filter) {
    ...
    matchLearningPatterns(request) {
        ...
        if (request.category) {
            ...
            results = results.filter(p => p.applicable_domains.some(d => d.toLowerCase() === cat) ||
                p.business_context.toLowerCase().includes(cat));
        }
        if (request.region && request.region !== 'All') {
            results = results.filter(p => !p.applicable_regions || p.applicable_regions.includes('All') || p.applicable_regions.includes(request.region));
        }
    ...
    getSupportingMemories(patternId, tenantId) {
        const pattern = this.getLearningPatternById(patternId);
        if (!pattern || !pattern.supporting_memory_ids)
            return [];
        const memories = pattern.supporting_memory_ids
            .map(id => memory_store_1.memoryRepository.getMemoryCaseById(id))
            .filter((m) => m !== null);
...
exports.InMemoryLearningPatternRepository = InMemoryLearningPatternRepository;
exports.learningPatternRepository = new InMemoryLearningPatternRepository();
