...
exports.simulateEnterpriseSignalTimelines = simulateEnterpriseSignalTimelines;
const index_1 = require("../../../packages/contracts/src/index");
const GENERATOR_VERSION = 'sim_gen_v1.0.0';
function simulateEnterpriseSignalTimelines(request) {
    const context = request.context;
    // Validation
    const contextVal = (0, index_1.validateSignalSimulationContext)(context);
    if (!contextVal.valid) {
        throw new Error(`Invalid SignalSimulationContext: ${contextVal.errors.join(', ')}`);
    }
    const fromPeriod = request.temporal_range?.from || 'T-90';
    const toPeriod = request.temporal_range?.to || 'T+30';
    const rangeVal = (0, index_1.validateTemporalRange)(fromPeriod, toPeriod);
    if (!rangeVal.valid) {
        throw new Error(rangeVal.error);
    }
    const fromIdx = index_1.ORDERED_SIMULATION_PERIODS.indexOf(fromPeriod);
    ...
    const slaFlexEffectiveIdx = slaFlexIntervention ? index_1.ORDERED_SIMULATION_PERIODS.indexOf(slaFlexIntervention.effective_period) : 999;
    ...
    if (scenarioFamily === 'promotion_surge' || context.scenario_id.includes('PROMO')) {
        ...
        timelines.push({
            ...
            observations: targetPeriods.map(period => {
                const periodIdx = index_1.ORDERED_SIMULATION_PERIODS.indexOf(period);
                let deltaPct = 0;
                if (periodIdx < index_1.ORDERED_SIMULATION_PERIODS.indexOf('T-7'))
                    deltaPct = 0;
                else if (period === 'T-7')
                    deltaPct = Math.round(promoLift * 0.4);
                else if (period === 'T-5')
                    deltaPct = Math.round(promoLift * 0.65);
                else if (period === 'T-3')
                    deltaPct = Math.round(promoLift * 0.85);
                else if (period === 'T-2')
                    deltaPct = Math.round(promoLift * 0.90);
                else if (period === 'T-1')
                    deltaPct = Math.round(promoLift * 0.95);
                else if (period === 'Today')
                    deltaPct = Math.round(promoLift * 0.90);
                else if (period === 'T+1')
                    deltaPct = Math.round(promoLift * 1.10);
                else if (period === 'T+3')
                    deltaPct = Math.round(promoLift * 1.25);
                else if (period === 'T+7')
                    deltaPct = Math.round(promoLift * 1.40);
                else if (period === 'T+30')
                    deltaPct = Math.round(promoLift * 0.30);
                return {
                    period,
                    observed_at: now,
                    effective_at: now,
        ...
        timelines.push({
            ...
            observations: targetPeriods.map(period => {
                ...
                let deltaPct = 0;
                if (period === 'T-7')
                    deltaPct = Math.round(promoLift * 0.3);
                else if (period === 'T-5')
                    deltaPct = Math.round(promoLift * 0.55);
                else if (period === 'T-3')
                    deltaPct = Math.round(promoLift * 0.80);
                else if (period === 'T-2')
                    deltaPct = Math.round(promoLift * 0.95);
                else if (period === 'T-1')
                    deltaPct = Math.round(promoLift * 1.10);
                else if (period === 'Today')
                    deltaPct = Math.round(promoLift * 1.20);
                else if (period === 'T+1')
                    deltaPct = Math.round(promoLift * 1.35);
                else if (period === 'T+3')
                    deltaPct = Math.round(promoLift * 1.50);
                else if (period === 'T+7')
                    deltaPct = Math.round(promoLift * 1.60);
                else if (period === 'T+30')
                    deltaPct = Math.round(promoLift * 0.20);
                return {
                    period,
                    observed_at: now,
                    effective_at: now,
