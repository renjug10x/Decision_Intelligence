...
### ADR-073: One Canonical Decision Case, Declared Once and Derived Everywhere (`DEMO-HARD-01`)
- **Status:** Approved and **implemented** (2026-09-15). Enforced by `tests/unit/run-canonical-scenario-tests.ts`, whose assertions compare two independent surfaces against each other rather than restating either one's arithmetic back to it.
- **Context:** Demand, Promotion and Campaign Decision told a consistent *story* on incompatible *arithmetic*. Demand published a £269.4K exposure derived from an estate-wide projection at £2.07 a unit. Promotion and Campaign Decision worked from an abstract 10,000-unit week and a £1.85 unit contribution that no price in the catalogue supported. Decision Ripple published a £595,200 revenue lift against a £480,000 baseline that existed nowhere else in the product. Each surface was internally consistent — which is exactly why none of them looked wrong on its own, and why a reader moving between them was left to infer that the revenue scale, the store count, the supplier and the product had all changed underneath the same decision. For a product whose proposition is that decisions and consequences are connected, three economic universes sharing a vocabulary is not a cosmetic defect.
- **Decision, part 1 — there is ONE decision case, and it is a record, not a convention.** `packages/contracts/src/canonical-scenario-model.ts` declares the scenario's identity, estate, calendar, demand, supply, inventory and economics, and exposes every derived quantity as a function. No connected surface may restate a declared value as its own literal, and no surface may derive a second basis for a quantity the record already answers. `deriveUnitEconomics` is the shape of the rule: it still *accepts* a projection-derived unit revenue, and uses it only where it diverges from the canonical basis by more than a declared tolerance, because a material divergence is a finding about the scenario rather than something to paper over.
- **Decision, part 2 — supply is declared as RATIOS of the demand base, never as counts.** The supplier allocation is an index (1.10) and the flex clause is a rate (12% of un-promoted weekly demand). A count belonging to one population is precisely how the same lever came to recover 1,200 units on one surface and 84,000 on another; a ratio rescales with the scenario and cannot. The whole case therefore resizes from one number — `base_demand_units_per_week` — and the surfaces cannot drift apart again without a test failing.
- **Decision, part 3 — the declared plan scale and the measured run rate are two quantities, and their agreement is asserted rather than arranged.** The record declares 350,000 units a week; the Demand surface independently measures 349,998 from the observed history. Making one read the other would have destroyed the only evidence that the model and the data describe the same estate. They are asserted to agree within 0.01% instead.
- **Decision, part 4 — supplier promotional funding is part of the economics.** Without it, a 20% cut on a line earning about 42% at list is value-destroying at every depth, no depth is recommendable, and the Promotion surface has nothing to say. That is not a modelling artefact: price investment on a staple is part-funded by the supplier, and the funded share is the single largest determinant of whether a promotion is worth running. It was absent, and its absence was being compensated for by a contribution-erosion rate of 0.7% per point of depth — under which a 20% price cut cost 14% of margin. The erosion rate is now derived from list price, unit cost and the funding agreement, and the funding is visible as a declared assumption.
- **Decision, part 5 — elasticity means one thing.** The Promotion surface read ε 2.4 as 2.4pp of demand per point of depth; the causal engine read the same 2.4 as 0.55pp. Both cited the same SKU and the same category, and a reader comparing the two screens was looking at a fourfold disagreement about what a discount does. One declared rate now serves both, and cannibalisation scales with the volume the mechanic creates rather than being charged as a flat −1.2pp to a 5% cut and a 30% one alike.
- **Consequences:** Every value in the pre-hardening demonstration's demand narrative is *preserved exactly*, including the post-intervention conclusion — recorded in [`COGNIX_PRESENTATION_SYNC_DELTA.md`](../reports/COGNIX_PRESENTATION_SYNC_DELTA.md) — because the demand engine's arithmetic was already sound and only its inputs belonged to a different world. The promotion economics move materially and the narrative moves with them: the committed 20% national cut is no longer *"destroys £3,300"* but *"adds £8.1K where 14% adds £43.5K, and strands 130,129 units"*. That is a weaker commercial argument on its own and a considerably stronger decision argument, because the supply consequence becomes the reason to challenge the plan. Several existing acceptance suites asserted the retired 10,000-unit scale as literals; each now asserts the relationship it was really protecting, and the outcome-frontier fixtures search for a declaration that binds rather than naming one calibrated against a population that no longer exists.
...
**Approved and implemented (`SCI-01`, 2026-09-16).** The derivations are now pure functions of a
scenario (`scenario*(scenario, …)` in `packages/contracts/src/canonical-scenario-model.ts`), with the
protected reference bound by name at one place per quantity (`canonical*(…)`). The connected journey's
227 call sites read the bound layer unchanged, which is why §1 and §2 of
`COGNIX_PRESENTATION_SYNC_DELTA.md` reconcile to the digit. The original ruling declared *one canonical decision case*, and at
...
- **Decision, part 1 — every modelled value is held in GBP and no upstream code knows what currency a reader selected.** Conversion happens in one display layer whose entry point takes an *amount in the base currency*. The parameter name is the contract and the type system carries it: there is no code path along which an already-converted amount can re-enter, so double conversion is structurally impossible rather than merely avoided. Converting to the base currency is the identity, not a multiplication by a rate that happens to be 1.
- **Decision, part 2 — the provider is an interface and the default needs no secret.** Rates are the European Central Bank's reference rates, served through Frankfurter. It requires no API key, so there is no credential that could be inlined into a client bundle, written into an image or logged; a tenant with its own treasury rates implements `FxRateProvider` and nothing else changes. The browser never calls the rate host: it reads the platform's own cached endpoint, so rates are fetched once per process rather than once per render and no third-party host sees traffic from a client demonstration.
- **Decision, part 3 — a failed rate lookup costs provenance, never the screen.** The service returns last-known-good first and a dated seeded reference set second, and carries the reason forward so the currency control can say what the reader is looking at. The endpoint answers 200 with a usable rate set and reports degradation in the payload, because a currency control that fails closed takes the whole surface with it. A rate that arrives absent, zero or non-finite is treated as a *failed fetch*, not as a zero rate — returning it would multiply every pound on the surface by nothing.
- **Decision, part 4 — units are not money.** Unit counts, percentages, points, scores, hours, days and store counts are never converted. The prose transform that localises sentences the engines composed with pounds already in them is deliberately narrow: it matches an amount with a currency symbol in front of it and nothing else, and it returns the sentence untouched when the display currency is the base, so a GBP reader never sees a re-rounded value. That transform is a display-layer accommodation, not a modelling step — the structural fix is for those engines to emit amounts and let the surface compose the sentence, and it is recorded as open work rather than hidden behind the helper.
- **Consequences:** Switching currency re-renders against rates already in hand and cannot fail or stall. Browser acceptance in USD and EUR finds no surviving pound sign, no mixed currencies and no unit or percentage altered on any surface of the connected journey. In an environment with no outbound access to the rate host — this one included — the demonstration runs identically on the dated fallback and says so, which is the behaviour the capability was specified to have.


---

### ADR-075: Two Demand Numbers, One Story — The Bridge Between Price Depth and a Configured Campaign (`DEMO-HARD-02`)
- **Status:** Approved and **implemented** (2026-09-15). Enforced by §8 of `tests/unit/run-canonical-scenario-tests.ts`, which asserts the relationship rather than equality, and asserts the bound on the divergence where the two legitimately differ.
- **Context:** The Promotion surface published `+46.8%` from the elasticity curve and `+50.5pp` from the causal engine for the same campaign, with nothing saying they were different quantities. The instinct is to force them equal. That would have been wrong, and decomposing the engine's drivers showed why: its `mechanic_response` net of `portfolio_effects` is **46.81pp**, against the curve's **46.8%** — the same quantity, computed by two engines, agreeing to a rounding unit. The remaining **4.4pp** is `audience_response`, `place_response` and `temporal_response`. A curve over depth has to hold audience, placement and timing fixed or it is not a curve; a causal engine that ignored them would not be causal. Two correct answers to two different questions, presented as one metric.
- **Decision, part 1 — publish the bridge, do not collapse it.** `CampaignDemandBridge` states `price_depth_response_pp + campaign_design_response_pp = total_attributable_pp`, names each design component, and is asserted to reconcile. The surface reads *"This campaign as configured: +50.5 pp — +46.8pp from the price cut, +3.7pp from who we target, where we run it, when we run it."* A client can now see that the two numbers are consecutive, not contradictory, and can interrogate the difference rather than discount both.
- **Decision, part 2 — a rate and a total are different numbers and both get named.** `CDI-02` trajectories are a per-period WEEKLY rate, deliberately: `CDI-05` plots them period by period and its own disclosure forbids summing them. The counterfactual nonetheless carried `horizon_days: 14` beside weekly volumes, so a weekly contribution sat on the same screen as a 14-day curve value with nothing distinguishing them. `CampaignEconomicBasis` publishes `rate_period_days`, `campaign_window_days` and the campaign-window totals. The weekly basis is unchanged, because it is correct for what it serves.
- **Decision, part 3 — where the two legitimately diverge, the divergence is bounded and asserted.** The engine applies a declared ±6% SKU-and-region differentiation band that a national curve does not model, so at a regional scope the depth responses differ. The test asserts exact agreement at the scenario's own scope and a bounded difference elsewhere, which is what stops a per-configuration modifier growing into a second economic model unnoticed.
- **Consequences:** No slide value changed. `Contribution impact: +£11.4K` becomes `Contribution over 14 days: +£22.7K` — the same economics, correctly stated over the campaign window. Two calendar defects surfaced and closed with it: the campaign window was built from `Date.now()` while the forecast horizon was anchored to the scenario clock, so the two drifted apart by a day for every day that passed; and it was counted exclusively, so a fourteen-day campaign ran fifteen days.

---

### ADR-076: An Archetype May Behave Differently; It May Not Be Priced Differently (`DEMO-HARD-02`)
- **Status:** Approved and **implemented** (2026-09-15). Enforced by §9 of `tests/unit/run-canonical-scenario-tests.ts`, which runs over EVERY entry in the archetype map rather than over a chosen one.
- **Context:** `DEMO-HARD-01` put the canonical decision case on one economic framework and left the six alternative archetypes on the retired 50-store, £1.85-contribution estate. A presenter switching archetype mid-demonstration left the unified economics behind in one click, and the surface would have shown a "national" play reaching fifty shops. Worse, each archetype's `discovery` headline carried its own seeded contribution: one asserted **+£4,850** above a curve whose best point lost **£12,527**.
- **Decision, part 1 — the framework is applied to the MAP, not to each definition.** `canonicaliseArchetypes` runs over every seeded archetype on the way to the exported map, so there is no path to a selectable scenario that skips it and no way for an archetype added later to arrive outside it. What is derived: unit contribution at each depth, every tier's contribution, every play's contribution, every play's store scope, the discovery headline, the recommended tier and any supplier-funding condition's value. What stays seeded and untouched: elasticity, demand response, cannibalisation, supplier constraint, seasonality, inventory condition, competitive response, supply exposure, waste impact and narrative.
...
- **Status:** Approved and **implemented** (`SCI-01`, 2026-09-16). Enforced by the identity and source guards in `tests/unit/run-canonical-scenario-tests.ts` §11, and generalised per scenario under `SCI-02`. `ScenarioFamilyId` and `ArchetypeId` are now declared as `taxonomy` on the scenario record; `packages/contracts/src/scenario-registry.ts` is the one place an identity resolves; `requireScenarioId()` refuses a request that names no scenario with `HTTP 400`. Three sources of scenario identity were removed: the route literals `SCN-PROMO-01` / `promotion_surge`, the decision state's default of the same, and `SCN-${archetype.id}`, which the archetype projection minted and which resolved to nothing.
...
- **Status:** Approved and **implemented** (`SCI-01`, 2026-09-16). `packages/contracts/src/scenario-clock.ts` owns the contract, including the `SimulationPeriod` → scenario-day mapping; the signal generator and the ESF-2 simulator stamp every observation through it. Civil time is retained where ADR-078 part 2 says it belongs and is now taken through a named `platformReceiptNowIso()` so a deliberate use reads as deliberate. Measured after implementation: two consecutive `GET /api/v1/signals` reads are byte-identical including `observed_at` and `effective_at`, and a `T-7` observation is seven scenario days old.
...
- **Status:** Approved and **implemented** (`SCI-01`, 2026-09-16). `skuContextFactor` and its `hashSeed` are gone; `CanonicalScenario.differentiation` declares scope multipliers with mandatory reasons and depth-response anomalies with theirs. §8 of the canonical suite now asserts EXACT agreement of the price-depth response at every scope, and the divergence that remains is attributed driver by driver to named design components.
- **What retirement exposed, recorded because it changed a published value.** The seeded elasticity curve carried 2.34pp of net demand per point of depth. The record declares 2.4pp gross and an 8% cannibalisation rate, which is 2.208pp net. The missing 6% was the hash: the curve had been calibrated against the causal engine WITH `skuContextFactor` applied, and at the one scope it was calibrated for — `"National"` — that hash happened to be 1.06. So the two surfaces agreed at exactly one scope by arithmetic coincidence and diverged by up to six per cent everywhere else. The curve is now derived from the record's declared terms, which moves the Promotion surface's published depth response from 46.8% to 44.16% and the committed plan's 14-day contribution from +£8.1K to −£5.2K. `COGNIX_PRESENTATION_SYNC_DELTA.md` §3.1 is updated. **Demand → Promotion → Campaign Decision §1 and §2 are unchanged to the digit**, because they never read the curve.
...
- **Status:** Approved and **implemented** (`SCI-02`, 2026-09-16). Specified in [`COGNIX_SCENARIO_CERTIFICATION.md`](../governance/COGNIX_SCENARIO_CERTIFICATION.md). The contract is `packages/contracts/src/scenario-certification-model.ts`; the twelve dimension evaluators are `lib/scenario-certification.ts`; the gate installs into the `SCI-01` activation seam and is reached through `lib/scenario-runtime.ts`, which every server route now resolves scenarios through. `SCN-FRESH-DAIRY-CHEDDAR-001` is `CERTIFIED` on all twelve dimensions across 84 executed checks.
- **Part 3 in code, because a declared verdict is only as good as what stops it being asserted.** A dimension's verdict is DERIVED from its checks and never written: `deriveDimensionVerdict` returns `FAIL` if any applicable check failed, `NOT_APPLICABLE` if none ran, and `PASS` only where at least one ran and all passed. `validateCertificationResult` then refuses a result whose verdict does not match what its evidence earns, whose `FAIL` or `NOT_APPLICABLE` carries no reason, whose dimension set is incomplete, or whose assertion count overstates what executed — and `certificationStateOf` reads an inadmissible result as `FAILED`, never as certified. Applicability is per CHECK as well as per dimension, each carrying its own reason, because a scenario with no observed history has nothing to reconcile its declared scale against while the clock checks beside it remain entirely applicable.
- **What implementation proved about part 4.** The gate was run against the reference scenario before anything else, and it failed `C-8` at 47,093 against 72,450. The gate was wrong: the check had omitted the scenario's own declared substitution-recovery share. Corrected, the reference scenario certifies. That is §5 of the certification record working as intended — *if the gate cannot certify it, the gate is wrong and is corrected* — and it is recorded here because a gate that had been quietly exempted instead would have been worth nothing.
...
- **Status:** Approved and **implemented** (`SCI-05`, 2026-09-17), reactivating `ESF-4`. The engine is `lib/living-evidence-engine.ts`; the contracts it implements were declared and frozen at Gate A in `packages/contracts/src/living-evidence-contracts.ts` and are unchanged. Refresh advances the as-at marker one `SimulationPeriod` through `simulateEnterpriseSignalTimelines`, re-evaluates every published quantity and publishes a delta whose `decision_consequence_statement` is mandatory and non-null. Materiality is derived by leave-one-out and banded, never scored (part 5, with ADR-072). Determinism is asserted three ways in `tests/unit/run-sci05-living-evidence-tests.ts`, including byte-identity under three host timezones. **Part 3 held under pressure:** two of the three scenarios' first advance moves nothing material, and the delta says so rather than manufacturing movement.
...
- **Status:** Approved and **implemented** (`SCI-01`, 2026-09-16). `packages/contracts/src/provenance-vocabulary.ts` declares `origin` / `method` / `authority` and maps every existing enum onto it; nothing was rewritten to satisfy it, which is ADR-082 part 2 honoured rather than merely quoted. The scenario record carries a descriptor, and `validateProvenanceDescriptor` refuses `drafted` + `authoritative` so ADR-044's rule is enforced by the type rather than remembered by each consumer. Extended in use by `SCI-05` and `SCI-07`.
...
- **Status:** Approved and **implemented**, across four convergence gates: Gate A `8d6d960c`, Gate B `e0a9c23d`, Gate C `c7c9f64f`, Gate D `2f8d7ed8` (2026-09-22). Authorised 2026-09-15 as the execution discipline for the `SCI` programme. Recorded in [`COGNIX_SCENARIO_INTELLIGENCE_WORK_PACKETS.md`](../governance/COGNIX_SCENARIO_INTELLIGENCE_WORK_PACKETS.md). **Part 4 is what has earned the ADR its keep.** At Gate B the two lanes shared two files and were nonetheless publishing one scenario's economics under three identities (`R-35`). At Gate C they shared exactly one file and `SCI-06` was rendering a thousand lines of its own contract-shaped fixture. At Gate D they again shared exactly one file, and five defects were waiting in files only ONE lane had touched — among them a module that opened by declaring it introduced no second economic engine and was one, and a surface publishing an exposed gap of 130,125 units beside the money at risk for 181,437. **A changed-file overlap of one is not an integration of one**, and no lane suite has ever found these: they are properties of the converged estate and only a gate evaluates those. **Part 2 has also been load-bearing in the other direction:** at Gate D a residual that appeared to need a deregistration seam on a FROZEN `SCI-01` contract (`R-SCI07-5`) was closed inside the consuming lane instead, because measurement showed an unregistered pre-flight was a complete discriminator. A convergence event is a licence to change an owning module, not an invitation to change a contract.
...
### ADR-085: One Scenario Authority — the Gated Scenario Runtime Owns the Registry; `cognix-world` and the Browser Consume Records, Never Register Them (`SCI-07R`)
- **Status:** Approved and **implemented** (`SCI-07R`, 2026-09-25). Closes the `SCI-08` entry blocker `R-SCI07-6`; closes `R-32`; narrows `R-28`. Measured on the production build in `service` mode: an authored scenario is listed, selected and executed under one id (HTTP 29/29, browser 58/58 at 1440/1024/720); it survives a `cognix-world` restart and — as decided — not a BFF restart; `run-sci07r-scenario-registry-authority-tests.ts` 63/63; full estate 4,259 passed with `R-25`'s `A6b` the only failure. Design and evidence: [`COGNIX_SCI_07R_SCENARIO_REGISTRY_AUTHORITY_REPORT.md`](../reports/COGNIX_SCI_07R_SCENARIO_REGISTRY_AUTHORITY_REPORT.md). **No frozen contract changes.**
- **Context:** The `SCI-01` registry is a module-level `Map`. Every process that imports the contracts barrel holds its own copy, bootstrapped from compiled data: the BFF, `cognix-world` and the browser. They agreed only because nothing existed that had not been compiled. The first authored scenario broke that coincidence in five measured ways: in `service` mode the catalogue was `cognix-world`'s and could not see a BFF-registered scenario; the BFF could nonetheless activate it, publishing an active id absent from its own catalogue; signal generation was refused by world on one route and silently fell back in-process on another; the browser registry never learned the record, so client engines computed the previous scenario in every mode; and a confirmed scenario was visible to every tenant. `R-32` (activation) and `R-28` (certification) were earlier symptoms of the same root cause.
- **Decision, part 1 — one authority.** The gated scenario runtime in the BFF (`lib/scenario-runtime.ts` over the frozen `SCI-01` registry) is the only component that registers, certifies, activates or publishes the catalogue, in every `COGNIX_WORLD_MODE`. It is where the gate, the authoring domain and every economic engine already live.
- **Decision, part 2 — `cognix-world` is stateless for scenarios.** It serves no catalogue (`410`, naming the authority) and resolves no scenario identity for a product request: scenario-scoped signal generation is record-carrying (`POST /api/v1/signals/snapshot`), with the record the BFF resolved. A non-OK upstream in `service` mode is an explicit error, never a fallback.
- **Decision, part 3 — the browser registry is a governed read projection.** It receives an authored record only from `GET /api/v1/scenarios/record`, which serves only a registered, certified scenario visible to the caller's tenant; it never replaces a record it holds and grants no activation.
- **Decision, part 4 — tenant visibility is deny-by-default.** Compiled scenarios are visible to all; an authored scenario only to the tenant that confirmed it; anything else to no one. Invisible is indistinguishable from unregistered on every route. Ownership is held by the authoring domain, not added to the frozen registry. `tenant_id` remains self-declared: this is scoping, not authentication.
- **Decision, part 5 — persistence is deferred on correctness grounds.** All authored state lives in one process and is lost together on BFF restart, so the estate is never incoherent; it is not durable. The BFF must run as a single instance. Revisit before horizontal scaling, a non-demonstration tenant, or a `SCI-08` requirement for restart survival; the path is a store the BFF alone writes and hydrates from (`R-SCI07R-1`).
- **Consequences:** Selection, catalogue and execution agree across the topology the demo runs on. The lifecycle becomes *author → resolve → certify candidate → register → certify authoritatively → confirm → catalogue → select → project → execute*; `project` is not an authority. No economic quantity is computed by any new code. `cognix-world` can no longer hold an uncertified scenario, but it still cannot verify one it is handed (`R-SCI07R-3`).

---

### ADR-086: An Attested Upload Enriches a Draft With Measured Inputs — It Is Not an Outcome Observation, and It Never Certifies, Confirms or Activates (`SCI-10`)
- **Status:** Approved as a **design decision**; **not implemented**. Declared at the `SCI-10` reconciliation design gate, 2026-09-25, against `SCI-08` `aad33e90`. The contract is declared in [`COGNIX_ATTESTED_UPLOAD_CONTRACT.md`](../governance/COGNIX_ATTESTED_UPLOAD_CONTRACT.md); the reconciliation is [`COGNIX_SCI_10_RECONCILIATION_DESIGN_GATE.md`](../reports/COGNIX_SCI_10_RECONCILIATION_DESIGN_GATE.md). Builds on ADR-080, ADR-082, ADR-083, ADR-085 and ESF-6.
- **Context:** `SCI-10` was defined before `SCI-07R` and `SCI-08` as *"admit user CSV as scenario enrichment through the `ESF-6` attested-observation path"*. Read against the code, ESF-6 admits **realised outcome actuals** (`REALISED_COMMERCIAL_ACTUAL`, `REALISED_OPERATIONAL_ACTUAL`) as `OutcomeObservation`s, which `CDI-08` prediction comparison resolves from their receipts to judge a committed decision. A scenario's weekly demand, store count or stock cover is a **pre-decision input**, not the outcome of a decision. Admitting inputs as outcomes would place non-outcome data where the learning loop reads outcomes. Meanwhile the frozen contracts already anticipate the right target: the Provenance Vocabulary carries `origin: 'attested'`, readiness maps attested evidence to `Ready`, and the Scenario Draft contract types fourteen fields as `MEASUREMENT`, *"`Limited` until a file or an attested source supplies it (`SCI-10`)"*.
- **Decision, part 1 — the target is the draft.** An admitted upload writes a closed set of directly observable `MEASUREMENT` inputs into a `DRAFT` of the same tenant, reduced by two declared rules (`MEAN_OF_PERIODS`, `LATEST_PERIOD`). The field's provenance is derived as `attested` while its value equals the admitted value. Every consumer reads it exactly as it reads a stated input: no second path, engine or registry (ADR-085).
- **Decision, part 2 — ESF-6's trust primitives are reused; its observation store is not.** `SourceAttestation` (a named person, a statement, `FIRST_PARTY_OPERATOR_ATTESTATION`), server-issued prefixed identifiers, `checkNoReservedServerFields`, server-derived `synthetic_demo` and the per-tenant monotonic `ServerReceipt` are reused unchanged, with one additive receipt kind, `SCENARIO_UPLOAD_ADMISSION`. Nothing is admitted as an `OutcomeObservation`.
- **Decision, part 3 — the lifecycle is not shortened by attestation.** Upload, validation, attestation and admission are four separate operations, and admission is the last one an upload owns. Certification, confirmation, activation and execution stay exactly where ADR-080, ADR-083 and ADR-085 put them. An attestation is a first-party operator declaration, **not a proof**, and is never rendered as verification.
- **Decision, part 4 — withdrawal needs a governed unset.** Withdrawing an admitted upload must return its fields to CogniX's declared assumption. The authoring domain has no way to express "no longer stated" (`R-SCI08-2`), so a governed unset is added to the authoring domain's update operation, as `SCI-10`'s first slice. It is an implementation change in `lib/scenario-authoring/`; the frozen Scenario Draft contract already represents an unset field as an absent key.
- **Decision, part 5 — no new persistence.** Uploads live in-process with the lifetime of the draft they enrich. Raw cell values are discarded at admission. Reproducibility is met by the person's own file, because admission is a pure function of bytes, mapping, product and scenario Today. `R-SCI07R-1` is unchanged.
- **Decision, part 6 — GenAI is optional.** Column mapping is proposed deterministically by header matching and confirmed by a person. A GenAI mapping proposal may be offered, receiving headers, inferred types and at most three redacted samples, never raw cells. It is advisory, and `SCI-10`'s acceptance runs with the provider off.
- **Consequences:** An uploaded extract can make a capability `Ready` for the first time, which is the value `SCI-07` reserved for `SCI-10`. It does so without letting data certify itself, without adding a store, and without contaminating the outcome evidence `CDI-08` relies on. An exported draft keeps admitted values but not their attested provenance: a copied number is not an attested one.
