...
- `POST /api/v1/campaigns/decision-contract/[id]/validity` — Decision Half-Life validity assessment (`DELIVERED` — CDI-07A; computation over caller-supplied `as_of` / context; never a resource write). Aspirational planning path `GET /api/v1/campaigns/validity` is superseded by this POST (CDI-05 §8.2 precedent).
...
| `DecisionContract` / `DecisionValidityAssessment` | Domain Contract | **DELIVERED** — CDI-07A | Immutable decision contract + evidence-backed Half-Life validity states (`packages/contracts/src/campaign-decision-contract-model.ts`). |
...
| 11 | Decision Contract & Intent Fusion | What assumptions bound this decision? | `DecisionContract` | Intent, frontier, counterfactual/causal refs, constraints | Records an already-resolved decision with verbatim basis snapshots + assumptions/triggers | `POST /api/v1/campaigns/decision-contract` (DELIVERED) | Decision Contract registration (Canvas Layer 7) | Decision Contract Summary Card | Immutable Decision Contract Store + WP10-C `decision_contract_ref` | Verifies RJ-C* eligibility, digest binding, CDI-01 non-mutation, WP10-C idempotent register | `CDI-07A` | COMPLETED |
| 12 | Decision Half-Life | Is this decision still valid? | `DecisionValidityAssessment` | Contract assumptions/triggers, optional ESF signal context | Evidence-backed validity states — not countdown/expiry/decay | `POST /api/v1/campaigns/decision-contract/[id]/validity` (DELIVERED) | Tier 1 Validity Indicator + Tier 2 Evidence Drawer | Compact Validity Indicator $\to$ Assumption / Signal Validity Evidence | Enterprise Signal Fabric (`ESF-1`/`ESF-2`) | Verifies precedence, fabricated-STABLE refusal, SCENARIO_DRIVEN ≤ WATCH, quantitative half-life NOT_AVAILABLE | `CDI-07A` | COMPLETED |
| 13 | Campaign Pre-Mortem | How could this decision fail? | `CampaignPreMortem` | Declared readiness/constraint/assumption evidence | Enumerates failure modes from declared evidence — no likelihood/impact scores | `POST /api/v1/campaigns/decision-contract/[id]/pre-mortem` (DELIVERED) | Pre-Mortem panel (Canvas Layer 8) | Failure modes by consequence_order | Decision Contract Store | Verifies RJ-P*, no invented risk precision, no alternative proposal | `CDI-07B` | COMPLETED |
| 14 | Prediction vs Reality & Learning | What differed, and what is reusable? | `PredictionOutcomeComparison` / `LearningCandidate` | Contract snapshots + ESF-3 observations | Like-for-like comparison only; eligibility conjunction; WP10-D memory when eligible | `POST …/prediction-comparison` + `…/learning-candidate` (DELIVERED) | Prediction vs Reality + Learning panels | Comparability-first surface; full LE-* conjunction | `cognix-learning` (WP10-D) | Verifies AC-20/21 fail-closed, no SUCCESS/FAILURE, X1–X4 | `CDI-07B` | COMPLETED |
