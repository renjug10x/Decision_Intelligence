...
- `POST /api/v1/campaigns/timeline` — Contextual Decision Timeline projection endpoint (`PLANNED`).
- `POST /api/v1/campaigns/outcome-frontier` — Multi-Objective Outcome Frontier solver endpoint (`PLANNED`).
...
| **`CDI-05`** | Decision Timeline & Progressive Decomposition | Multi-lens timeline & `What? → Why? → Evidence → What If?` driver decomposition. | **HARD:** `CDI-02`. **INTEGRATION:** `CDI-04`. **ENHANCEMENT:** `CDI-03`. |
...
| 7 | Decision Timeline | What happens over time? | `DecisionTimelineProjection` | Forecast trajectory, constraints, events | Projects temporal timeline across 4 lenses | `POST /api/v1/campaigns/timeline` (PLANNED) | Decision Timeline Primary Canvas View | Contextual Decision Timeline | Shared Decision State (`WP10-C`) | Verifies timeline rendering across 4 lenses | `CDI-05` | PROPOSED |
| 8 | Curiosity Demand Decomposition | Why is this outcome predicted? | `DemandDecomposition` | Driver contributions (mechanic, weather, etc) | Separates demand lift into explicit pp contributions | `POST /api/v1/intent-fusion/evaluate` (EXISTS) | `What? → Why? → Evidence → What If?` Drawer | Driver Contribution Waterfall | Intent Fusion Engine (`IFI-01`) | Verifies sum of driver pp equals total predicted uplift | `CDI-05` | PROPOSED |
| 9 | Multi-Objective Outcome Frontier | Is there a better strategy? | `OutcomeFrontier` | Revenue, contribution, waste, availability | Solves Pareto-efficient strategy trade-off frontier | `POST /api/v1/campaigns/outcome-frontier` (PLANNED) | Strategy Trade-off Card Grid | Outcome Frontier Strategy Cards | Shared Decision State Store | Verifies Pareto dominance calculations across 4 strategy plays | `CDI-06` | PROPOSED |
| 10 | AI-Generated Competing Strategies | What alternatives exist? | `CampaignScenario` | Deterministic strategy play templates | Generates structured plays (Scenario 0 + alternatives) | `POST /api/v1/campaigns/competing-plays` (PLANNED) | Competing Strategy Drawer | Scenario Comparison Matrix | Gemini AI Narrative Wrapper | Verifies deterministic play generation without LLM dependency | `CDI-06` | PROPOSED |
