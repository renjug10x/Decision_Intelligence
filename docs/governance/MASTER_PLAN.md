...
- **CDI-07B — Campaign Pre-Mortem & Closed Learning Loop [DESIGN FROZEN]:** Sub-slice internal evidence gates (`CDI-07B.1` Pre-Mortem Resilience, `CDI-07B.2` Prediction vs Reality Comparison, `CDI-07B.3` Enterprise Memory/Learning Integration). Three separate artefacts, each bound to a `DecisionContract` by `contract_id` **and** `contract_digest`, none of which mutates the contract or supersedes it. A pre-mortem enumerates failure modes from declared evidence with no likelihood, probability or impact score. A comparison reports a prediction error only where grain and measurement basis match; the contracted attributable prediction is never differenced against a gross observation, and there is no `SUCCESS` or `FAILURE` verdict. Learning eligibility is a deterministic conjunction; one eligible case never becomes a pattern, and no automatic `EnterpriseLearningPattern` creation is authorised. No quantitative Decision Half-Life is produced from CDI-07B evidence.
  ...
  - *Design gate:* `docs/reports/COGNIX_CDI_07B_LEARNING_LOOP_DESIGN_GATE.md`.
  - *Hard Dependencies:* `CDI-07A` (`DecisionContract`), `WP10-D` (`cognix-learning`).
  - *Integration Dependencies:* `ESF-3` (External Signal Connectors).
  - *Enhancement Dependencies:* None.
