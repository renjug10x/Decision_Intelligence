...
- **`ATL-06C` — AI Interpretation & Hybrid Reasoning [NOT STARTED]:** The third ADR-048 evidence class as a reasoning capability — provider-generated interpretation constrained to rest on cited governed statements and admitted market claims, refusing where a premise cannot be cited, degrading to the `ATL-06A` templated form with the provider off. Gated separately from market evidence deliberately: a sourced claim and a judgement drawn from it are different risks, and collapsing them is how a market expectation becomes a capability claim.
